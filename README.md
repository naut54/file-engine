...
| `.analyze(path)` | `analyze` | Read-only tree inspection — counts, sizes, largest files, extension/age breakdowns | `.extensions()`, `.exclude_globs()`, `.min_size()` / `.max_size()`, `.modified_after()` / `.modified_before()`, `.max_depth()`, `.follow_symlinks()`, `.detect_mime_types()`, `.detect_duplicates()`¹, `.top_n_largest()`, `.estimate_total()`, `.on_error()` | `AnalysisHandle` → `AnalysisReport` |
