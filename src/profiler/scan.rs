...
/// Bounds on `scan()`'s walk itself, as opposed to `threshold` (which
/// only classifies what the walk already found). Defaults match the
/// walk's original hardcoded behavior exactly — unbounded depth, never
/// follow symlinks — so every caller that doesn't need either keeps
/// passing `ScanOptions::default()` with no change in behavior.
#[derive(Debug, Clone, Copy, Default)]
pub(crate) struct ScanOptions {
    /// Passed straight through to `WalkDir::max_depth`: the scanned
    /// root is depth 0, its immediate children depth 1, and so on.
    /// `None` means unbounded.
    pub(crate) max_depth: Option<usize>,
    /// Passed straight through to `WalkDir::follow_links`. See
    /// `scan_blocking`'s comment on what this changes about symlink
    /// handling.
...
pub(crate) async fn scan(root: &Path, threshold: u64, options: ScanOptions) -> Result<Workload> {
    let root = root.to_path_buf();
    tokio::task::spawn_blocking(move || scan_blocking(&root, threshold, options))
        .await
        .expect("scan blocking task panicked")
}

fn scan_blocking(root: &Path, threshold: u64, options: ScanOptions) -> Result<Workload> {
    ...
    // With `options.follow_symlinks` false (the default), symlinks (to
    // files or directories) are skipped, not followed:
    ...
    //
    // With it true, walkdir resolves the link itself: `file_type()`
    // then reports the *target's* type, so a symlinked directory is
    // recursed into and a symlinked file is captured as a normal entry
    // — and a cycle created by doing so surfaces as an `Err` from the
    // iterator (handled by `classify_walkdir_error` below) rather than
    // an infinite walk.
...
mod tests {
    ...
    async fn single_file_root_produces_one_entry_workload() {
        ...
        let workload = scan(&file_path, 256, ScanOptions::default()).await.unwrap();
    ...
    async fn directory_with_mixed_sizes_classifies_correctly() {
        ...
        let workload = scan(dir.path(), 256, ScanOptions::default()).await.unwrap();
    ...
    async fn nonexistent_path_returns_error_instead_of_panicking_or_empty_workload() {
        ...
        let result = scan(&missing, 256, ScanOptions::default()).await;
    ...
    #[tokio::test]
    async fn max_depth_prunes_traversal_past_the_bound() {
        let dir = tempdir().unwrap();
        fs::write(dir.path().join("root.txt"), b"a").unwrap();
        fs::create_dir(dir.path().join("nested")).unwrap();
        fs::write(dir.path().join("nested").join("child.txt"), b"b").unwrap();

        let options = ScanOptions {
            max_depth: Some(1),
            ..ScanOptions::default()
        };
        let workload = scan(dir.path(), 256, options).await.unwrap();
        ...
        let names: Vec<_> = workload
            ...
            .collect();
        assert_eq!(names, vec![PathBuf::from("root.txt")]);
    }
    ...
    async fn follow_symlinks_true_walks_into_a_symlinked_directory() {
        ...
        let workload = scan(root_dir.path(), 256, options).await.unwrap();
    ...
    async fn follow_symlinks_true_reports_a_cycle_as_an_error_instead_of_hanging() {
        ...
        let result = scan(dir.path(), 256, options).await;
